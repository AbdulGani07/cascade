...
export function registerDashboardCommand(program: Command): void {
  program
    ...
    .option("--base <ref>", "attach Git impact from this base ref")
    .option("--head <ref>", "Git comparison head ref", "WORKING_TREE")
    .option("--output <file>", "write the dashboard dataset to a JSON file")
    .option("--output-only", "write the dataset and exit without starting a server")
    .option("--url-output <file>", "write the tokenized local URL for capture automation")
    ...
    .action(
      (
        ...
        try {
          ...
          const analysis = analyze(absPath);
          if (options.base) {
            analysis.gitImpact = analyzeGitImpact(absPath, {
              base: options.base,
              head: options.head,
            });
          }
          const result = JSON.parse(toJson(analysis)) as typeof analysis;
          if (options.output) {
            const outputPath = path.resolve(options.output);
            fs.mkdirSync(path.dirname(outputPath), { recursive: true });
            fs.writeFileSync(outputPath, `${JSON.stringify(result, null, 2)}\n`, "utf8");
            printSuccess(`Dashboard dataset written to ${options.output}`);
          }
          if (options.outputOnly) {
            if (!options.output) {
              printError("--output-only requires --output <file>.");
              process.exitCode = 2;
            }
            return;
          }
          ...
          server.listen(4000, "127.0.0.1", () => {
            ...
            if (options.urlOutput)
              fs.writeFileSync(path.resolve(options.urlOutput), `${url}\n`, "utf8");

            if (options.open !== false) void open(url);
          });
