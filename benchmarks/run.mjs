...
import { analyze, detectProjectIntelligence } from "../packages/core/dist/index.js";
...
for (const [name, count] of sizes) {
  ...
  for (let index = 0; index < count; index++) {
    ...
    fs.writeFileSync(
      path.join(source, index === 0 ? "index.ts" : `module-${index}.ts`),
  ...
  console.log(
    `| ${name} | ${result.nodes.length} | ${result.edges.length} | ${elapsed.toFixed(1)} | ${heapDelta.toFixed(1)} |`
  );
...
const languageCases = [
  ...
  [
    "Go",
    createGoPlugin(),
    (index) => `\"example.com/benchmark/type${index}\"\n`,
    "package main\nimport (\n",
    ")\nfunc main() {}",
  ],
  ...
  [
    "Vue",
    createVuePlugin(),
    (index) => `import Type${index} from "./Type${index}.vue"\n`,
    "<script>\n",
    "</script>",
  ],
  [
...
for (const [language, plugin, importLine, prefixOrSuffix, optionalSuffix] of languageCases) {
  ...
  const content = optionalSuffix
    ? `${prefixOrSuffix}${imports}${optionalSuffix}`
    : `${imports}${prefixOrSuffix}`;
  const extension =
    {
      "C#": "cs",
      "C++": "cpp",
  ...
  const context = {
    filePath: `benchmark.${extension}`,
    relativePath: `benchmark.${extension}`,
    content,
  };
  const started = performance.now();
